 ...
 * [com.ldzs.recyclerlibrary.strategy.GroupingStrategy]
...
class PullToRefreshStickyRecyclerView @JvmOverloads constructor(context: Context, attrs: AttributeSet? = null, defStyleAttr: Int = 0) : PullToRefreshRecyclerView(context, attrs, defStyleAttr) {
    ...
    override fun onLayout(b: Boolean, left: Int, top: Int, right: Int, bottom: Int) {
        ...
        stickyView?.let { it.layout(left, 0, right, it.measuredHeight) }
    }
    ...
    override fun setAdapter(adapter: RecyclerView.Adapter<*>) {
        ...
        if (adapter !is StickyCallback<*>) {
            ...
            listener = StickyScrollListener(adapter as StickyCallback<*>)
            targetView.addOnScrollListener(listener)
            if (null == observer) {
                observer = AdapterDataObserver(adapter as StickyCallback<*>)
            } else {
